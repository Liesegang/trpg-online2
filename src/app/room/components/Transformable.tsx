...
const ScaleHandle: React.FC<ScaleHandleProps> = ({ position, handler, scale }) => {
  return (
    <div className={`absolute cursor-nwse-resize ` + getPositionClass(position)} onMouseDown={handler}>
      <div className="w-2 h-2 border-1 border-white rounded-full bg-green-500" style={{ transform: `scale(${1 / scale}` }} />
    </div>
  );
};
...
const RotateHandle: React.FC<RotationHandleProps> = ({ handler, scale }) => {
  return (
    <div className={`absolute cursor-grab -top-5 left-1/2 -translate-x-1/2 -translate-y-1/2`} onMouseDown={handler}>
      <div className="w-2 h-2 border-1 border-white rounded-full bg-red-500" style={{ transform: `scale(${1 / scale}` }} />
    </div>
  );
};
...
const TransformHandles: React.FC<TransformHandlesProps> = ({ onScale, onRotate, scale }) => {
  return (
    <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 }}>
      <ScaleHandle handler={(e) => { e.stopPropagation(); onScale(e.clientX, e.clientY); }} position='bottom-center' scale={scale} />
      <ScaleHandle handler={(e) => { e.stopPropagation(); onScale(e.clientX, e.clientY); }} position='top-center' scale={scale} />
      <ScaleHandle handler={(e) => { e.stopPropagation(); onScale(e.clientX, e.clientY); }} position='left-center' scale={scale} />
      <ScaleHandle handler={(e) => { e.stopPropagation(); onScale(e.clientX, e.clientY); }} position='right-center' scale={scale} />
      <ScaleHandle handler={(e) => { e.stopPropagation(); onScale(e.clientX, e.clientY); }} position='top-left' scale={scale} />
      <ScaleHandle handler={(e) => { e.stopPropagation(); onScale(e.clientX, e.clientY); }} position='top-right' scale={scale} />
      <ScaleHandle handler={(e) => { e.stopPropagation(); onScale(e.clientX, e.clientY); }} position='bottom-left' scale={scale} />
      <ScaleHandle handler={(e) => { e.stopPropagation(); onScale(e.clientX, e.clientY); }} position='bottom-right' scale={scale} />
      <RotateHandle handler={(e) => { e.stopPropagation(); onRotate(e.clientX, e.clientY); }} scale={scale} />
    </div>
  );
};
...
const Transformable: React.FC<TransformableProps> = ({ children }) => {
  ...
  const handleScale = useCallback((clientX: number, clientY: number) => {
    ...
    const box = transformRef.current?.getBoundingClientRect() || { left: 0, right: 0, top: 0, bottom: 0 };
  ...
  const handleRotate = useCallback((clientX: number, clientY: number) => {
    ...
    const startRotation = Math.atan2(clientY - position.y, clientX - position.x) * (180 / Math.PI);
  ...
  return (
    <div
      ref={transformRef}
      className={clsx(
        "cursor-grab", "absolute", "inline-block", {
    ...
    >
      ...
      {showHandles && <TransformHandles onScale={handleScale} onRotate={handleRotate} scale={scale} />}
      <div>
